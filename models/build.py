...
from .mss_swin_transformer import SwinTransformerMSS
from .mss_moby import MoBYMSS


vit_models = dict(deit_small=deit_small_patch16_224,)
...
def build_model(config):
    ...
    if encoder_type == 'swin':
        ...
        enc = partial(
            ...
            use_checkpoint=config.TRAIN.USE_CHECKPOINT,
            norm_befor_mlp=config.MODEL.SWIN.NORM_BEFORE_MLP,
            is_mask=config.MODEL.MSS.IS_MASK, mask_ratio=config.MODEL.MSS.MASK_RATIO,
        )
    ...
    if model_type == 'moby':
        encoder = enc(num_classes=0, drop_path_rate=config.MODEL.MOBY.ONLINE_DROP_PATH_RATE)
        encoder_k = enc(num_classes=0, drop_path_rate=config.MODEL.MOBY.TARGET_DROP_PATH_RATE)
        model = MoBY(cfg=config, encoder=encoder, encoder_k=encoder_k,
        ...
        encoder = enc(num_classes=0, drop_path_rate=config.MODEL.MOBY.ONLINE_DROP_PATH_RATE)
        model = MoBYMSS(cfg=config, encoder=encoder)
