...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, unmask=None):
        """
        Args:
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, unmask=None):
        ...
        if unmask is None:
            ...
            unmask_x_windows = x_windows[batch_range, unmask]
            unmask_x_windows = self.norm1(unmask_x_windows)
            unmask_attn_windows = self.attn(unmask_x_windows, unmask=unmask)  # nW*B, window_size*window_size, C

            unmask_shortcut = window_partition(shortcut.view(B, H, W, C), self.window_size)
            unmask_shortcut = unmask_shortcut.view(-1, self.window_size * self.window_size, C)
            unmask_shortcut = unmask_shortcut[batch_range, unmask]
            unmask_windows = unmask_shortcut + self.drop_path(unmask_attn_windows)
            unmask_windows = unmask_windows + self.drop_path(self.mlp(self.norm2(unmask_windows)))

            # merge windows
            attn_windows = torch.zeros_like(x_windows)
            attn_windows[batch_range, unmask] = unmask_windows
            attn_windows = attn_windows.view(-1, self.window_size, self.window_size, C)
            x = window_reverse(attn_windows, self.window_size, H, W)  # B H' W' C
            x = x.view(B, H * W, C)
            pass
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, mlp_ratio=4.,
                 ...
                 norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, norm_before_mlp='ln'):
    ...
    def forward(self, x, unmask=None):
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, unmask=unmask)
            else:
                x = blk(x, unmask=unmask)
...
class Projector(nn.Module):
    ...
    def __init__(self, in_dim, pro_dim, num_patches, depth, heads, dim_head):
        ...
        self.projector = Transformer(dim=pro_dim, depth=depth, heads=heads, dim_head=dim_head, mlp_dim=pro_dim * 4)
    ...
    def forward(self, x, unmasked_indices, masked_indices):
        ...
        mask_tokens = mask_tokens + self.pos_emb(masked_indices)  # 所有掩掉的块
...
class SwinTransformerMSS(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, norm_before_mlp='ln', is_mask=False, mask_ratio=0.5, **kwargs):
        super().__init__()
        ...
        self.patch_embed = PatchEmbed(img_size=img_size, patch_size=self.patch_size, in_chans=in_chans,
    ...
    def forward_features(self, x):
        device = x.device
        if self.is_mask:
            x_b, x_c, x_w, x_h = x.size()
            p1, p2 = x_w // self.window_size, x_h // self.window_size

            image_num_patch = (x_w // p1) * (x_h // p2)  # 空间上多少个块
            ...
            final_id_unmask = rand_indices[:, num_mask:]
            final_id_unmask_2 = rand_indices[:, :image_num_patch - num_mask]
            final_id_mask = rand_indices[:, :num_mask]
            final_id_mask_2 = rand_indices[:, image_num_patch - num_mask:]
            pass
        ###########################################################################################################
        ...
        if self.is_mask:
            x_1 = self.layers[-1](x, unmask=final_id_unmask)
            x_2 = self.layers[-1](x, unmask=final_id_unmask_2)

            batch_range = torch.arange(x_1.shape[0], device=device)[:, None]
            x_1 = x_1[batch_range, final_id_unmask]
            x_2 = x_2[batch_range, final_id_unmask_2]
            x_1_pro = self.projector(x_1, unmasked_indices=final_id_unmask, masked_indices=final_id_mask)
            x_2_pro = self.projector(x_2, unmasked_indices=final_id_unmask_2, masked_indices=final_id_mask_2)

            if self.has_predictor:
                x_1_pro = self.projector_bn_last(x_1_pro)
                x_2_pro = self.projector_bn_last(x_2_pro)
                x_1_pre = self.predictor(x_1_pro)
                x_2_pre = self.predictor(x_2_pro)
            else:
                h1, h2 = self.projector_bn_last(x_1_pro), self.projector_bn_last(x_2_pro)
                x_1_pre, x_2_pre = h1.argmax(dim=1), h2.argmax(dim=1)
                pass
            return x_1_pro, x_2_pro, x_1_pre, x_2_pre
        else:
            x = self.layers[-1](x)
